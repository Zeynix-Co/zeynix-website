...
// POST /api/admin/forgot-password - Send admin password reset email
export async function POST(request: NextRequest) {
    try {
        ...
        const user = await User.findOne({ email, role: 'admin' }).select('+resetPasswordToken +resetPasswordExpires');
        if (!user) {
        ...
        return NextResponse.json(
            {
                success: false,
                message: 'Internal server error during admin password reset request'
            },
            { status: 500 }
        );
    }
